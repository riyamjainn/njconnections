        ...
        {words.map(word => (
          <button
            ...
            className={\`p-2 border rounded text-sm font-medium transition
              ...
              \${Object.entries(solvedGroups).some(([, group]) => group.includes(word)) ? 'opacity-50 cursor-default' : ''}
            \`}
            disabled={Object.entries(solvedGroups).some(([, group]) => group.includes(word))}
          >
            {word}
          </button>
        ))}
